...
@router.post("/qr-codes/", response_model=QRCodeResponse, status_code=status.HTTP_201_CREATED, tags=["QR Codes"])  # Fixed path
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    qr_code_full_path = Path(QR_DIRECTORY) / qr_filename
    ...
    if qr_code_full_path.exists():
        ...
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,  # Updated to return 409 for conflicts
...
async def list_qr_codes_endpoint(token: str = Depends(oauth2_scheme)):
    ...
    qr_files = list_qr_codes(Path(QR_DIRECTORY))
    responses = [
        QRCodeResponse(
...
@router.delete("/qr-codes/{qr_filename}", status_code=status.HTTP_204_NO_CONTENT, tags=["QR Codes"])  # Fixed parameter name
async def delete_qr_code_endpoint(qr_filename: str, token: str = Depends(oauth2_scheme)):
    ...
    qr_code_path = Path(QR_DIRECTORY) / qr_filename
